...
### Fixed
- `fix/workflow-id-cli`: workflow_id() inline hash fallback for CLI sourcing — when state-lib.sh is sourced directly from CLI without log.sh, project_hash() is undefined causing workflow_id() to return "_main" instead of "{hash}_main"; added inline SHA-256 computation with PROJECT_ROOT/CLAUDE_DIR/git fallback chain, normal hook path unchanged; 10 new tests (DEC-WORKFLOW-ID-CLI-001, closes #239)
- `feature/fix-check8-require-state`: Add missing require_state() to pre-bash.sh Check 8 and Check 10 proof gates — without it, proof_state_get() was undefined in SQLite-only environments, causing silent fallback to stale flat files (DEC-STATE-DOTFILE-003, closes #237)
