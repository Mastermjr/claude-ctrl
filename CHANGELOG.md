...
- `feature/state-unify-w5-1`: State Unification W5-1 — remaining hook migrations (compact-preserve, log, prompt-submit, session-init, session-lib, stop) to SQLite state_emit/require_state; removes type guard fallbacks from migrated hooks; event emission for session lifecycle and hook activity (DEC-STATE-UNIFY-005 through 010, #219)
...
- `feature/tester-verification-integrity`: Two-Tier Verification Protocol for tester agent — T1 (tests pass) and T2 (feature works in production context) now explicitly tracked in coverage tables and AUTOVERIFY blockers; implementer.md gains Production Reality Check checklist (DEC-TESTER-TIER-001, DEC-IMPL-PRODCHECK-001), 5 tests
- `feature/tester-verification-w2`: Tier 2 evidence backstop for auto-verify — post-task.sh mechanically verifies Coverage table contains T2 "Fully verified" rows before granting auto-verify signal, fixes bash 3.2 negative subscript bug that silently killed secondary validation pipeline (DEC-AV-TIER2-001), 7 new tests
- `feature/governance-efficiency-w2`: Cross-hook signal deduplication Wave 2 — cached git/plan state producers in session-init.sh with `_cached_git_state`/`_cached_plan_state` consumer API in git-lib.sh and plan-lib.sh, wired into all 8 consumer hooks; 15 new tests, all deny gates preserved (DEC-EFF-012 through DEC-EFF-014, #209)
...
- `feature/session-label`: Session-specific statusline Line 3 — when agents are dispatched, Line 3 now shows the worktree branch name or agent description instead of the static initiative, letting concurrent sessions display unique identifiers; 4 decisions (DEC-SESSION-LABEL-001 through 004), 12 new tests
...
- `feature/sqlite-w1`: SQLite WAL-based state store (Wave 1) — rewrite hooks/state-lib.sh with sqlite3 backend (state_update, state_read, state_cas, state_delete, workflow_id), WAL mode with busy_timeout=5000ms, per-workflow isolation via workflow_id column, SQL injection prevention, lattice-enforced CAS; 20-test suite in test-sqlite-state.sh (DEC-SQLITE-001 through DEC-SQLITE-010, closes #128, #129)
- `feature/lint-on-write`: Multi-language lint-on-write hook (hooks/lint.sh) — synchronous PostToolUse on Write/Edit runs shellcheck/ruff/go vet/cargo clippy with CI-matching exclusions, 3s cooldown per file; 38 lint-scope tests (DEC-LINT-001)
- `feature/dispatch-enforcement`: Gate 1.5 in pre-write.sh blocks orchestrator from writing source code directly — compares CLAUDE_SESSION_ID against .orchestrator-sid to detect orchestrator context (DEC-DISPATCH-001, DEC-DISPATCH-002, DEC-DISPATCH-003)
...
- `feature/v4-w1-1`: Lint cooldown consolidation + orchestrator SID flat-file removal + gitignore hardening — per-file `.lint-cooldown-*` sentinels replaced with single `tmp/.lint-cooldowns` file (lint.sh, pre-write.sh, session-end.sh, session-init.sh), orchestrator SID flat-file I/O removed from session-init.sh (SQLite KV sole authority), 11 runtime state patterns added to .gitignore; 27 new tests, 10 updated (DEC-V4-LINT-COOLDOWN-001, DEC-V4-OSID-FLATFILE-001, DEC-V4-GITIGNORE-001, closes #243)
- `feature/kv-agent-findings`: Add SQLite audit trail for agent findings — 7 hook files emit state_emit("agent.finding") alongside flat-file .agent-findings writes; completes State Unification initiative (16/16 migratable dotfiles resolved) (DEC-STATE-KV-007)
- `feature/kv-test-status`: Migrate `.test-status` to SQLite KV store — test-runner.sh dual-writes via state_update("test_status"), 6 consumer hooks updated; 10 new KV tests in test-session-kv.sh (DEC-STATE-KV-005)
- `feature/kv-cost-history`: Migrate session cost history to SQLite — migration v3 adds `cost_usd REAL NOT NULL DEFAULT 0` to session_tokens; session-end.sh INSERT includes cost_usd (DEC-STATE-KV-004)
- `feature/kv-token-history`: Migrate token history to SQLite — session_tokens table with migration v2, session-end.sh dual-writes INSERT alongside flat-file append, statusline.sh drops per-render .session-main-tokens write (DEC-STATE-KV-003)
- `feature/kv-session-prompt`: Migrate `.session-start-epoch` and `.prompt-count-*` to SQLite KV store (DEC-STATE-KV-002)
- `feature/kv-orchestrator-sid`: Migrate `.orchestrator-sid` from flat-file to SQLite KV store — first KV dotfile-to-SQLite migration (DEC-STATE-KV-001)
- `feature/dispatch-inject`: Dynamic dispatch summary injection — DISPATCH.md maintains a delimited summary section that session-init.sh extracts and injects into session context at startup (DEC-DISPATCH-INJECT-001)
- `fix/autoverify-guardian-infer`: AUTOVERIFY reliability Wave 2 — Guardian inference-based fallback (INFER-VERIFY) for when AUTO-VERIFY-APPROVED is absent but tester evidence is clean; 5 validation criteria in guardian.md (DEC-AV-GUARDIAN-001, #196)
- `feature/autoverify-reliability`: AUTOVERIFY reliability Wave 1 — rewrite tester Auto-Verify Signal section with positive-default framing (emit CLEAN unless blockers apply), fix check-tester.sh Phase 2 audit, add post-task.sh inference check (DEC-TESTER-AUTOVERIFY-001, DEC-AV-MISS-001, #194, #195)
- `feature/dual-color-bar`: Dual-color context pressure bar — statusline now renders three visual regions (system overhead in dim, conversation usage in severity-colored, empty space), with fingerprint-based baseline capture (DEC-DUALBAR-001, DEC-DUALBAR-002)
- `feature/rsm-phase3`: Unified state directory with dual-write migration — `state_dir()` in state-lib.sh provides `state/{phash}/` per-project directories; proof-status, test-status, and session hooks migrated (DEC-RSM-STATEDIR-001)
- `feature/wave-planning-metrics`: Replace serial phase-based planning with DAG-based wave decomposition — Phase 3 output format uses dependency graphs to compute waves of parallelizable work; new per-item metrics (Weight S/M/L/XL, Gate none/review/approve, Deps W-ID list)
...
- `feature/v4-w1-1`: Resolve all 4 test failures + T05 pre-existing — db-guardian-lib.sh exempted from settings sync check (library, not hook), three shellcheck fixes in test files (SC2059, SC2004, SC2221/SC2222), T05 updated from removed dual-write contract to SQLite-only; test suite 372/372 pass
...
- `fix/workflow-id-cli`: workflow_id() inline hash fallback for CLI sourcing — added inline SHA-256 computation with PROJECT_ROOT/CLAUDE_DIR/git fallback chain; 10 new tests (DEC-WORKFLOW-ID-CLI-001, closes #239)
- `feature/fix-check8-require-state`: Add missing require_state() to pre-bash.sh Check 8 and Check 10 proof gates — without it, proof_state_get() was undefined in SQLite-only environments (DEC-STATE-DOTFILE-003, closes #237)
...
- `fix/statusline-lifetime-tokens`: Fix statusline lifetime tokens display — double-nesting path guard, time-based cache pruning (1-hour TTL), 4-line layout splitting metrics across two lines so lifetime tokens are never truncated; 21 new tests (DEC-STATUSLINE-4LINE-001, DEC-LIFETIME-TOKENS-001)
- `fix/guardian-stale-marker`: Fix Gate A.0 false blocks in task-track.sh — validate trace manifest status before denying Guardian dispatch; 7-test suite (DEC-STALE-MARKER-004)
- `feature/shellcheck-fixes`: Gate-denied trace records in task-track.sh — blocked agent dispatches now write `outcome: "gate-denied"` traces instead of phantom 0-duration entries that skewed observatory metrics (DEC-GATE-DENIED-001, #174)
- `fix/sourcelib-worktree-path`: Fix source-lib.sh path resolution in worktrees — add fallback to canonical `$HOME/.claude/hooks`; 2 new tests (DEC-SRCLIB-FALLBACK-001, #207)
- `fix/governance-self-bypass`: Close governance self-bypass vectors — extend pre-write.sh branch guard to governance-critical markdown (agents/*.md, docs/*.md, CLAUDE.md, ARCHITECTURE.md); 26 new test cases (DEC-RECK-011)
- `fix/statusline-column-collapse`: Reserve 65 chars for Claude Code right-panel in statusline width calculation (DEC-STATUSLINE-TERMWIDTH-003)
- `fix/statusline-baseline-pid`: Fix PID-scoped baseline filename — now uses single `.statusline-baseline` per workspace (DEC-DUALBAR-003)
- `fix/trace-null-projects`: Fix null project_name in trace index — finalize_trace() backfills null project fields at trace seal time; 10+11 test suite (#173)
- `fix/guardian-stale-marker`: Fix Gate A.0 false blocks — stale markers from crashed sessions no longer block legitimate Guardian dispatches
- `fix/ci-sqlite-tests`: Update 4 failing CI tests for SQLite WAL state backend
- `fix/lint-full-coverage`: Extend `--scope lint` from 34 hooks to 97 files matching CI's full shellcheck coverage; 101 total lint tests (#127)
...
- `feature/xplatform-reliability`: Portable `_file_mtime()` and `_with_timeout()` wrappers in core-lib.sh — replace 25 inline `stat -f %m` patterns across 12 files (DEC-XPLAT-001, DEC-XPLAT-002)
- `feature/backfill-null-fallback`: Fix CLAUDE_SESSION_ID always "unknown" — extract session_id from Claude Code hook stdin JSON in read_input() (DEC-SESSION-ID-001, #175)
- `feature/proof-sweep-marker-based`: Replace TTL-based proof-status sweep with marker-based ownership check — session-end.sh checks for .active-*-{phash} markers in TRACE_STORE instead of 4h mtime TTL (DEC-PROOF-SWEEP-001)
